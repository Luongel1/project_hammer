...
Chain 1: 
Chain 1:  Elapsed Time: 0.56 seconds (Warm-up)
Chain 1:                225.422 seconds (Sampling)
Chain 1:                225.982 seconds (Total)
Chain 1: 
...
Chain 2:                208.219 seconds (Sampling)
Chain 2:                208.689 seconds (Total)
Chain 2: 
...
Chain 3:                207.839 seconds (Sampling)
Chain 3:                208.361 seconds (Total)
Chain 3: 
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Define the chunk size (adjust based on available memory)}
\NormalTok{chunk\_size }\OtherTok{\textless{}{-}} \DecValTok{50000}  
...
\NormalTok{predictions }\OtherTok{\textless{}{-}} \FunctionTok{vector}\NormalTok{(}\StringTok{"numeric"}\NormalTok{, }\FunctionTok{nrow}\NormalTok{(analysis\_data\_test))}

\CommentTok{\# Loop through each chunk and generate predictions}
\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\SpecialCharTok{:}\NormalTok{num\_chunks) \{}
...
\CommentTok{\# Calculate MAE and RMSE for model evaluation}
...
\FunctionTok{kable}\NormalTok{(evaluation\_metrics, }\AttributeTok{col.names =} \FunctionTok{c}\NormalTok{(}\StringTok{"Evaluation Metric"}\NormalTok{, }\StringTok{"Value"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
